...
class GenerateStarkConfigTask extends SysTask<GenerateBuildConfig> {

    GenerateStarkConfigTask(TaskManager manager, ApkVariant variant, StarkVariantScope starkScope) {
        super(manager, variant, starkScope)
    }

    @Override
    protected GenerateBuildConfig getTask() {
    ...
    void afterExecute() {
        ...
        def buildHash = starkScope.getBuildHash()
        generator.addField("String", "BUILD_HASH", '"' + buildHash + '"')
        generator.generate()
        def buildHashFile = starkScope.getStarkBuildHashFile()
        buildHashFile.createNewFile()
        buildHashFile.write(buildHash)
    }

    class StarkConfigGenerator {
        public static final String STARK_CONFIG_CLASS_NAME = "StarkConfig"
        public static final String STARK_CONFIG_NAME = STARK_CONFIG_CLASS_NAME + ".java"

        final Set<Modifier> PUBLIC_FINAL = EnumSet.of(Modifier.PUBLIC, Modifier.FINAL)
        private static final Set<Modifier> PUBLIC_STATIC_FINAL =
                EnumSet.of(Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL)

        private final File mGenFolder
        private final String mStarkConfigPackageName
         ...
         * @param StarkConfigPackageName the package in which to create the class.
         */
        StarkConfigGenerator(@NonNull File genFolder, @NonNull String StarkConfigPackageName) {
            mGenFolder = checkNotNull(genFolder)
            mStarkConfigPackageName = checkNotNull(StarkConfigPackageName)
        }

        StarkConfigGenerator addField(
                @NonNull String type, @NonNull String name, @NonNull String value) {
            mFields.add(new ClassFieldImpl(type, name, value))
            return this
        }

        StarkConfigGenerator addItems(@Nullable Collection<Object> items) {
            if (items != null) {
                mItems.addAll(items)
            }
            return this
        ...
        /**
         * Returns a File representing where the StarkConfig class will be.
         */
        File getFolderPath() {
            return new File(mGenFolder, mStarkConfigPackageName.replace(".", File.separator))
        }

        File getStarkConfigFile() {
            File folder = getFolderPath()
            return new File(folder, STARK_CONFIG_NAME)
        }

        /**
         * Generates the StarkConfig class.
         */
        void generate() throws IOException {
            ...
            File StarkConfigJava = new File(pkgFolder, STARK_CONFIG_NAME)

            Closer closer = Closer.create()
            try {
                FileOutputStream fos = closer.register(new FileOutputStream(StarkConfigJava))
                ...
                writer.emitJavadoc("Automatically generated file. DO NOT MODIFY")
                        .emitPackage(mStarkConfigPackageName)
                        .beginType("StarkConfig", "class", PUBLIC_FINAL)
