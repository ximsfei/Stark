 ...
 * under the License.
 */
package com.ximsfei.stark.gradle.aapt
...
class Aapt {
    ...
    Aapt(File assetDir, toolsRevision) {
     ...
     * @param idMaps
     */
    void filterPackage(List retainedTypes, int pp, Map idMaps, Map libRefTable, Set outUpdatedResources) {
    ...
    /**
     * Reset id str for R.java list.
     *
     * @param rJavaList rJavaFile list
     * @param idStrMaps id str map
     */
    def resetRJavas(List<File> rJavaList, def idStrMaps) {
        for (File rJava : rJavaList) {
            def tempRJava = new File(rJava.absolutePath + ".tmp")
            if (tempRJava.exists()) {
                tempRJava.delete()
                tempRJava.createNewFile()
            }
            def writer = tempRJava.newOutputStream().newWriter()
            def is = rJava.newInputStream()
            def pattern = Pattern.compile("(0x7f\\w{6})")
            is.readLines().each {
            ...
            writer.close()
            is.close()
            FileUtils.copyInputStreamToFile(tempRJava.newInputStream(), rJava)
            tempRJava.delete()
        }
    }
...
//        editor.createAndroidManifest(options)
    ...
    private
    static void resetAllXmlPackageId(File dir, int pp, Map idMaps, Set outUpdatedResources) {
