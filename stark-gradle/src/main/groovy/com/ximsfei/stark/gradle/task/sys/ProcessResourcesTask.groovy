...
class ProcessResourcesTask extends SysTask<ProcessAndroidResources> {
    ...
    private def gIdMaps = [:]
    private def gIdStrMaps = [:]
    private def gRetainedTypes = []
    private def gRetainedStyleables = []
...
//                Plog.q("resources-*.ap_: $file")
    ...
    def fixResId(AppExtension android, File unzipApDir, File publicTxtFile) {
        ...
        if (gRetainedTypes != null && gRetainedTypes.size() > 0) {
            ...
            aapt.filterPackage(gRetainedTypes, gPackageId, gIdMaps, libRefTable, updatedResources)
            Plog.q "[${project.name}] slice asset package and reset package id..."

            aapt.resetRJavas(rJavaFileList, gIdStrMaps)
            Plog.q "[${project.name}] reset R.java files..."
        } else {
            noResourcesFlag = 1
            if (aapt.deleteResourcesDir(filteredResources)) {
                Plog.q "[${project.name}] remove resources dir..."
            }

            if (aapt.deletePackage(filteredResources)) {
                Plog.q "[${project.name}] remove resources.arsc..."
            }

            if (sourceOutputDir.deleteDir()) {
                Plog.q "[${project.name}] remove R.java..."
            }
        }
...
//        }
        ...
        if (retainedEntries.size() == 0 && retainedPublicEntries.size() == 0) {
            gRetainedTypes = [] // Doesn't have any resources
            return
        }
...
//            }
        ...
        gIdMaps = staticIdMaps
        gIdStrMaps = staticIdStrMaps
        gRetainedTypes = retainedTypes
        gRetainedStyleables = retainedStyleables
//        mRetainedResourceKeys = retainedResourceKeys

        gAllTypes = allTypes
        gAllStyleables = allStyleables
    }

    /**
     * Get reserved resource keys of project.
     * The unnecessary resource `mipmap/ic_launcher' and `string/app_name' are excluded.
     */
    private def getReservedResourceKeys() {
        Set<SymbolParser.Entry> outTypeEntries = new HashSet<>()
        collectReservedResourceKeys(outTypeEntries)
        def keys = []
        outTypeEntries.each {
    ...
    private def collectReservedResourceKeys(Set<SymbolParser.Entry> outTypeEntries) {
//        outTypeEntries.add(new SymbolParser.Entry("string", "app_name"))
//        outTypeEntries.add(new SymbolParser.Entry("mipmap", "ic_launcher"))
//        outTypeEntries.add(new SymbolParser.Entry("mipmap", "ic_launcher_round"))
    }
}