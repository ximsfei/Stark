...
class AaptUtils {
    ...
    static def unzipApAllFile(File parentFile, File apFile, String childDir) {
        File unzipApDir = new File(parentFile, childDir)
        unzipApDir.delete()
        ZipFile zipFile = new ZipFile(apFile)
        ZipEntry zipEntry
        Enumeration<ZipEntry> entry = zipFile.entries()
        while (entry.hasMoreElements()) {
            zipEntry = entry.nextElement()
            String fileName = zipEntry.getName()
            File outputFile = new File(unzipApDir, fileName)
